...
def doneesPourGraphs(filename : str)-> list[list]:
    ...
    startC2 = transformToMinutes(42)
    startC3 = transformToMinutes(18,2)
    while readedlist != []:
...
def main(filenames : list[str]):
    ...
    plt.title("Bureau ouverture fenetre apres une heure ")
    plt.savefig("Bureau1heureFenetreOuvete", dpi = 900)
    plt.show()
    
filenames = ["FGRACH1.TXT","FGRACH2.TXT","FGRACH3.TXT"]
main(filenames)
