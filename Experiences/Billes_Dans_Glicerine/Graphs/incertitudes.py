...
#incertidude totale de type a et type b est sqrt((incerA)**2 + (incerB)**2) ou incerA + incerB
...
def manDerivation(func : str):
    inc_func = 0
    x0, y0, x1,y1 = symbols('x0, y0, x1,y1')
    vars = [x0, y0, x1,y1]
    incertitude = 0.00005 #0.005cm == 0.00005 m
    for i in range(len(vars)):
        inc_func += diff(func, vars[i])* incertitude
    return inc_func

##########################################
print(manDerivation("sqrt((x0-x1)**2 + (y0-y1)**2)"))
##########################################
