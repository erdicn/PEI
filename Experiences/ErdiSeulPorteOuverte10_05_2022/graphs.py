...
def doneesPourGraphs(filename : str)-> list[list]:
    ...
    while readedlist != []:
        ...
        list_ppm.append(ppm)
        if filename[-5] =="3":
            times.append(time+ 0.25)
        if filename[-5] =="2":
            times.append(time + 1.27)
        if filename[-5] =="1":
            times.append(time + 2.5)
...
def main(filenames : list[str]):
    ...
    plt.title("Porte ouverte 1 personne dans la salle")
    plt.show()

filenames = ["ERDI1.TXT","ERDI2.TXT","ERDI3.TXT"]
main(filenames)