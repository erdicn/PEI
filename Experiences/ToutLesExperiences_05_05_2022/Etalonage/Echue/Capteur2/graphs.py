...
def doneesPourGraphs(filenameslist : list[str])-> list[list]:
...
def plotGraphWithMultipleDonees(nb_graph : int, filename : str):
    list_ppm,times= doneesPourGraphs(filename)
    plt.errorbar(times, list_ppm)
    """
...
def main(filenames : list[str]):
    ...
    plotGraphWithMultipleDonees(1, filenames)
