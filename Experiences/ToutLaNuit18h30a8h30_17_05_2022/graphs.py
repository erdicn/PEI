...
def plotGraph(nb_graph : int, filename : str,list_ppm,times, temperatures, err_ppm):
    ...
    plt.errorbar(times, list_ppm, yerr = err_ppm, elinewidth=0.01, markeredgewidth=2, fmt="o", markersize='0.5')
...
def main(filenames : list[str]):
    ...
    for i in range(len(filenames)):
        ...
        if i == 2: #cest le capteur 3
            for j in range(len(list_ppm)):
                list_ppm[j] = list_ppm[j] - sistematicErreurDuCapteur3
    ...
    plt.title("Tout ferme personnes dans la salle pendant 14 heures")
    plt.show()

filenames = ["NUITF1.TXT","NUITF2.TXT","NUITF3.TXT"]
main(filenames)