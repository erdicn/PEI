...
// int createFrame(int ServerId, int ClientId, float val1);
...
void loop() {
    ...
    //uint8_t data[] = "Client 1";
    //rf95.send(data, sizeof(data));

    //rf95.waitPacketSent();

    char pMyFrame[RH_RF95_MAX_MESSAGE_LEN];
    //pMyFrame = (char *) malloc(FRAME_LEN*sizeof(char));

    //To check if the char is empty
    if (pMyFrame != NULL){
      memset(pMyFrame, '\0', RH_RF95_MAX_MESSAGE_LEN);
      createFrame(pMyFrame, ServerId, ClientId, val1); //function to create the frame
    }
...
int createFrame(char* pFrame, int ServerId, int ClientId, float val1) {
  //Convert the float to str
  char val1_str[5];
  dtostrf(val1, 5, 3, val1_str);

  //Fills the frame with the data to send
  sprintf(pFrame, "HBXCO %d %d %s EBXCO", ServerId, ClientId, val1_str);
